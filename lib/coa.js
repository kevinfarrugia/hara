...
async function action(args, opts, command) {
  ...
  if (output) {
    ...
    output = input.map((n) => n.replace(regHARFile, OUTPUT_FILE));
  }
...
function getFilesDescriptions(config, dir, files, output) {
  const filesInThisFolder = files
    ...
    .map((name) => ({
      ...
      outputPath: PATH.resolve(output, name.replace(regHARFile, OUTPUT_FILE)),
    }));
...
async function writeOutput(input, output, data) {
  ...
  try {
    return FS.promises.writeFile(output, data, "utf8");
  } catch (error) {
    return await checkWriteFileError(input, output, data, error);
  }
}
